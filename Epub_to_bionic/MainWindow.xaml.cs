...
namespace Epub_to_bionic
{
    ...
    public sealed partial class MainWindow : Window
    {
        ...
        // Convert button click event to trigger EPUB to SpeedRead Reading conversion
        private async void ConvertButton_ClickAsync(object sender, RoutedEventArgs e)
        {
            ...
            if (File.Exists(filePath))
            {
                try
                {
                    await ConvertEpubToSpeedReadAsync(filePath); // Call your conversion logic
                }
                finally
                {
        ...
        private async Task ConvertEpubToSpeedReadAsync(string filePath)
        {
            try
            {
                ...
                // Process each chapter for conversion
                foreach (var chapter in epubBook.ReadingOrder)
                {
                    ...
                    {
                        // Apply conversion to the chapter content
                ...
                string newFilePath = Path.Combine(Path.GetDirectoryName(filePath),
                                                  Path.GetFileNameWithoutExtension(filePath) + "_bio.epub");
        ...
        // Function to transform text for SpeedRead
        private string TransformToSpeedReading(string text)
        {
